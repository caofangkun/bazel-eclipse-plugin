...
/**
 * This is a quick wizard page that ask the user for the various targets and
 * source path he wants to include.
 */
public class WorkspaceWizardPage extends WizardPage {
  ...
  public ImmutableList<String> getDirectories() {
    return DirectoryTreeContentProvider
        .getSelectPathsRelativeToRoot(directories);
  }
  ...
  // A simple helper to use lambdas to create a SelectionListener that does the
  // same action on all
  // cases.
  private static SelectionListener createSelectionListener(
      Consumer<SelectionEvent> f) {
  ...
  private void createWorkspaceSelectionControls() {
    ...
    workspaceRootButton.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        if (wr != null) {
          ...
          DirectoryTreeContentProvider.setFileTreeRoot(directories,
              new File(wr));
          try {
            completionProvider.setBazelInstance(Activator.getDefault()
                .getCommand().getInstance(new File(getWorkspaceRoot())));
    ...
    workspaceRootButton.setLayoutData(
        new GridData(GridData.END, GridData.CENTER, false, false));
  }

  private void setControlGridData(Control control, int horizontalSpan,
      int verticalSpan, boolean verticalGrow) {
    GridData data = new GridData(GridData.FILL,
        verticalGrow ? GridData.FILL : GridData.CENTER, true, verticalGrow);
    data.horizontalSpan = horizontalSpan;
    data.verticalSpan = verticalSpan;
    control.setLayoutData(data);
  }

  private void addLabel(String labelText, int horizontalSpan,
      int verticalSpan) {
    Label label = new Label(container, SWT.NONE);
    label.setText(labelText);
    GridData data =
        new GridData(GridData.BEGINNING, GridData.FILL, false, true);
    data.horizontalSpan = horizontalSpan;
    data.verticalSpan = verticalSpan;
  ...
  private void updateControls() {
    ...
    setPageComplete(enabled && (directories.getCheckedElements().length > 0)
        && (targets.getItemCount() > 0));
  }
  ...
  private void setAutoCompletion() {
    try {
      ...
      adapter = new ContentProposalAdapter(target, new TextContentAdapter(),
          completionProvider, ks, null);
      adapter
  ...
  private void createTargetTextField() {
    ...
    target.addKeyListener(new KeyAdapter() {
      public void keyReleased(KeyEvent ke) {
        if (ke.keyCode == '\r' && (ke.stateMask & SWT.SHIFT) != 0
